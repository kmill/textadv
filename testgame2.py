...
world.define_action("referenceable_objects", accumulator=append)
...
def my_something(input, i, ctxt, next) :
    if i < len(input) and input[i] == "woo" :
        return product([[MatchedObject(input[i:i+1], "whatchamacallit", 2)]],
                       next(input, i+1))
    return []
